...
0–7, occurs **472×** in the blob (= 59 groups of 8). Meaning unknown, but it is
...
**8 rows × 7 columns**, 50 occupied positions, zero collisions. Column 0 is
...
- The official `Appendix C - Product Specification` (p. 35) **does not state a
  button count at all**.
- The source that number came from also claimed "LCD 84 × 84 px", while the manual
  says **96 × 64**. That source is therefore unreliable.
